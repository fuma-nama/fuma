...
Since the documentation itself isn't the top priority of a product or library, we don't have too much time to maintain the docs.
...
Markdown, especially GitHub flavoured Markdown (GFM) is a common format for docs.
...
- **Avoid 'first... then...':** Same as the _Ordered List_ rule. People read from top to bottom, you don't need sequencers to show the steps. Instead, give each step a meaningful heading.
...
- **Automated Documentation:** Generate docs from the "source of truth" (e.g. code and OpenAPI Schema), this can highly reduce the difficulty to maintain a docs. Like [Fumadocs](https://fumadocs.vercel.app) generates code examples from the source code of example apps.
